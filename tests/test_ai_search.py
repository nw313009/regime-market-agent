...
- THE SYNCED COLUMNS EXIST IN SILVER. A typo'd column name here fails at sync time in the
  workspace, hours later and far from the edit, so it is checked against the silver projection.
...
from src.pipelines.silver_news import NEWS_ARTICLE_COLUMNS
...
def test_settings_qualify_the_names_with_the_configured_catalog():
    ...
    assert SETTINGS.source_table_fqn == "market_intel.silver.news_articles"
...
def test_a_second_catalog_needs_no_other_edit():
    ...
    assert other.source_table_fqn == "market_intel_dev.silver.news_articles"
...
def test_the_key_is_doc_id_because_the_table_is_grained_on_two_columns():
    ...
    assert "doc_id" in NEWS_ARTICLE_COLUMNS


def test_every_synced_column_exists_in_the_silver_projection():
    missing = sorted(set(SYNCED_COLUMNS) - set(NEWS_ARTICLE_COLUMNS))

    assert missing == [], f"columns_to_sync names columns silver does not build: {missing}"
...
def test_a_missing_index_is_created_as_a_triggered_delta_sync_index():
    ...
    assert spec.source_table == "market_intel.silver.news_articles"
